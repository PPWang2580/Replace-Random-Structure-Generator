...
# Purpose: Replace 57% of Mg with Si using Enhanced SA
...
DIST_TOLERANCE = 0.05  # Tolerance for neighbor distribution (2, 3, 4 Mg neighbors)
BAD_NEIGHBOR_TOLERANCE = 0.03  # Tolerance for Si atoms with 0/1 Mg neighbors
BASE_SEED = 1920  # Base seed for reproducibility of seed generation
# Output settings
FILE_PREFIX = "554_60"  # 文件夹前缀
...
def substitute_atoms_with_sa(struct, total_to_replace, original, new, target_dist):
    ...
    while adjustments < max_adjustments:
        bad_indices = [idx for idx in best_indices if get_neighbor_count(best_struct, idx, ORIGINAL_ELEMENT) < 2]
        if not bad_indices:
            break
        si_idx = random.choice(bad_indices)
        mg_candidates = [i for i in mg_indices if get_neighbor_count(best_struct, i, ORIGINAL_ELEMENT) >= 2]
...
def generate_table(counts, total, config_name, seed, struct):
    target_ratios = {4: 0.27, 3: 0.57, 2: 0.16, 1: 0.0, 0: 0.0}
    if total == 0:
...
def process_configuration(config_idx, structure, total_to_replace, original_element, new_elements, target_counts, bad_neighbor_tolerance, base_seed):
    ...
    sa_table = generate_table(sa_counts, sa_total, f"{FILE_PREFIX}_seed{config_seed}.xsd", config_seed, doped_structure)  # 添加 doped_structure
    
    sa_output_file = os.path.join(OUTPUT_DIR, f"{FILE_PREFIX}_seed{config_seed}.xsd")
    if sa_valid:
    ...
    mod_table = generate_table(mod_counts, mod_total, f"{FILE_PREFIX}_seed{config_seed}_th.xsd", config_seed, modified_structure)  # 添加 modified_structure
    
    mod_output_file = os.path.join(OUTPUT_DIR, f"{FILE_PREFIX}_seed{config_seed}_th.xsd")
    if mod_valid:
